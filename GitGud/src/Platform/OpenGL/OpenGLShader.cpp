...
namespace GitGud
{
	...
	OpenGLShader::OpenGLShader(const std::string& filePath) : _rendererId(0)
	{
...

		{
			...
			CreateProgram();
			GG_CORE_WARN("Shader creation took {0}ms", timer.ElapsedMilliseconds());
		}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			GG_CORE_ASSERT(Utils::ShaderTypeFromString(type), "Invalid shader type specified.");

			size_t nextLinePos = source.find_first_not_of("\r\n", eol);
			GG_CORE_ASSERT(nextLinePos != std::string::npos, "Syntax error");
	...
	void OpenGLShader::CompileOrGetVulkanBinaries(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		for (auto&& [stage, source] : shaderSources)
		{
			...
			else
			{
				...
				{
					GG_CORE_ERROR(module.GetErrorMessage(), "");
					GG_CORE_ASSERT(false, "");
				}
	...
	void OpenGLShader::CreateProgram()
	{
		...
		if (isLinked == GL_FALSE)
		{
			...
			GG_CORE_ERROR("Shader linking failed ({0}):\n{1}", _filePath, infoLog.data());
	...
	void OpenGLShader::Reflect(GLenum stage, const std::vector<uint32_t>& shaderData)
	{
		...
		GG_CORE_TRACE("OpenGLShader::Reflect - {0} {1}", Utils::GLShaderStageToString(stage), _filePath);
		GG_CORE_TRACE("    {0} uniform buffers", resources.uniform_buffers.size());
		GG_CORE_TRACE("    {0} resources", resources.sampled_images.size());

		GG_CORE_TRACE("Uniform buffers:");
		for (const auto& resource : resources.uniform_buffers)
		{
			...
			GG_CORE_TRACE("  {0}", resource.name);
			GG_CORE_TRACE("    Size = {0}", bufferSize);
			GG_CORE_TRACE("    Binding = {0}", binding);
			GG_CORE_TRACE("    Members = {0}", memberCount);
		}
	}
}